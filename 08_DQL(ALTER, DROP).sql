...
--1_2 컬럼 수정 (MODIFY)
--> 데이터 타입수정 : MODIFY 컬럼명 바꾸고자하는 데이트타입
--> DEFAULT 값 수정 : MODIFY컬럼명 DEFAULT 바꾸고자하는 기본값
...
--3 -1) 컬럼명 변경 : RENAME COLUME 기존 컬럼명 TO 바꿀 컬럼명
...
--DEPT TITLE -> DEPT_NAME변경
ALTER TABLE DEPT_TABLE RENAME COLUMN DEPT_TITLE TO DEPT_NAME;

--3_2) 제약조건명 변경 : RENAME CONSTRAINT 기존제약조건명 TO바꿀제약조건명
...
--------------------------------------------------------------------------------
--TRUNCATE : 테이블 초기화
--DROR과는 다르게 테이블의 데이터만을 전부 삭제하며 테이블의 초기상태로 돌려준다.
