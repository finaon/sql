...
SELECT JOB_CODE, COUNT(*) AS "사원수", COUNT(BONUS) AS "보너스", SUM(SALARY) AS "급여",
        MIN(SALARY) AS "최저급여", MAX(SALARY) AS "최고급여",  ROUND(AVG(SALARY)) AS "급여평균"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
SELECT SUBSTR(EMP_NO, 8, 1) AS "성별(1:남 2:여)" , COUNT(*)
FROM EMPLOYEE
...
FROM EMPLOYEE
GROUP BY DEPT_CODE
WHERE AVG(SALARY) >= 3000000; -- WHERE은 한 행을 검사한다 (그러므로 오류가 난다)


SELECT DEPT_CODE, ROUND(AVG(SALARY)) AS "SALARY"
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000; -- HAVING은 GROUP별로 검사해준다 
...
-------------------------------------------------------------------------------
--3. UNION ALL : 여러개의 쿼리결과를 무조건 다 더하는 연산자
...
--4.MINUS : 선행 SELECT결과에서 후행 SELECT 결과를 뺀 나마지(차집합)
