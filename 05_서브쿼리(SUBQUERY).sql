...
/*
    ...
    < ALL (서브쿼리) : 여러개의 모든 결과값들 보다 작을 경우 조회
        비교대상 > ALL (서브쿼리의 결과값 => 값2 AND 비교대상 > 값3....)
        비교대상 > 값1 AND  비교대상 > 값2 AND 비교대상 > 값3...
        
*/            

--1) 유재식 또는 윤은해사원과 같은 직급인 사원들의 사번, 사원명, 직급코드, 급여 조회
--1_1) 유재식 또는 윤은해사우너의 직급코드
SELECT JOB_CODE
FROM EMPLOYEE
...
--2-2) 대리직급이면서 위의 결과값 보다 하나라도 큰 사원
...
--박나라 사원의 같은 직급코드, 같은 사수를 가지고 있는 사원들의 사번, 사원명, 직급코드, 사수번호 조회
--
...
--> 각직급별 최소급여를 받는 사원 조회(사번, 사원명, 직급코드, 급여)
SELECT  EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, (SALARY*12 + (SALARY * NVL(BONUS, 0))) AS "보너스포함연봉", DEPT_CODE
FROM EMPLOYEE
WHERE (SALARY*12 + (SALARY * NVL(BONUS, 0))) >= 3000000;

SELECT ROWNUM, EMP_ID, EMP_NAME, (SALARY*12 + (SALARY * NVL(BONUS, 0))) AS "보너스포함연봉", DEPT_CODE
FROM EMPLOYEE
WHERE (SALARY*12 + (SALARY * NVL(BONUS, 0))) >= 3000000
