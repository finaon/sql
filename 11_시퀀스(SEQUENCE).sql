/*
    ...
    [INCREMENT BY 숫자] --> 몇씩 증가시킬 건기 [기본값 1]
...
--[참고] 현재 계정이 소유하고 있는 시퀀스들의ㅏ 구조를 보고자할 때
...
SELECT SEQ_EMPNO_CURRVAL FROM DUAL; --에러
--> NEXTVAL 를 한번도 실행하지 않은 이상 CURRVAL을 실행할 수 없음
...
--보편적으로 NEXTVAL을 이용한다 -> 고객을 계속 생성을 하는게 보통이기 때문
...
/*
    ...
    [MAXVALUE 숫자]--> 최대값 지정[기본값은 매우큰수];
    [MINVALUE 숫자] --> 최소값 지정 [기본값 1] -- 순환하기 때문에 수정가능하다
    [CYCLE | NOCYCLE] --> 값의 순환여부 지정 [기본값 NOCYCLE]
    [NOCACHE | CACHE 바이트크기] --> 캐시메모리 할당(기본값 CACHE 20)
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
    VALUES(SEQ_EID.NEXTVAL, '김새똥', '111111-1111111', 'J6', SYSDATE);
