/*
    ...
    (주자 사용하는 SELECT문을 저장해두면 긴 SELECT 문을 매번 다시 기술할 필요가 없다.)
    임시테이블 같은 존재(실제 데이터가 담겨있는건 아니다 => 논리적인 테이블)
*/
...
/*
    *뷰 컬럼에 별칭부여
    서브쿼르의 SELECT절에 함수식이나 산술연산식이 기술되어있을 경우 반드시 별칭을 지정해야한다.
*/
...
/*
    * DML 명령어로 조작이 불가능한 경우가 많다.
    ...
    1) 뷰에 정의도어있지 않느 ㄴ컬럼을 조작하려고 하는경우
    2) 뷰에 정의되어 있지 않은 컬럼중에 베이스테이블 상에 NOT  NULL제약조건이 지정되어있는 경우
    3) 산술연산식 또는 함수식으로 정의가 되어있는 경우
    4) 그룹함수나 GROUP BY절이 포함된 경우
    5) DISTINCT구문이 포함된 경우
    6) JOIN을 이용해서 여러테이블을 연결시켜놓는 경우 
...
/*
    ...
    [상세표현식]
    CREATE [OR REPLACE] [FROCE | NOFORCE] VIEW 뷰명
    AS 서브쿼리
        [WITH CHECK OPTION]
        [WITN READ ONLY];
        
    1) OR REPLACE : 기존에 동일한 뷰가 있을 경우 갱신하고, 존재하지 않을경우 새로 생성해라
    2) FORCE | NOFORCE
    ...
    4) WITH READ ONLY : 뷰에 대해서 조회만 가능하로고
*/
...
--서브쿼리에 기술된 테이블이 존재하지 않나도 뷰가 우선은 생기게 된다.
...
--200만원사원 급여를 200만원으로 변경(SALARY >= 3000000조건에 맞지 않는 변경)
...
CREATE OR REPLACE VIEW VW_EMP
AS SELECT EMP_IDM, EMP_NAME, BONUS
    FROM EMPLOYEE
