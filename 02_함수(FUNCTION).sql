...
SELECT ROUND(123.656, 0) FROM DUAL; -- 기본자리수는 소수점 첫번째 자리에서 반올림 : 0
SELECT ROUND(123.456, 1) FROM DUAL; -- 양수로 증가할 수록 소수점뒤로 한칸씩 이동
SELECT ROUND(126.456, -1) FROM DUAL; -- 음수로 감소할 수록 소수점 앞자리로 이동


/*
    *CEIL
    올림처리를 위한 함수(위치값 지정 X <-오류남)
...
-- *MONTHS BETWEEN : 두 날짜 사이의 개월 수
-- 사원들의 사원명, 입사일, 근무일수, 근무개월수를 조회
SELECT EMP_NAME, HIRE_DATE, FLOOR(SYSDATE - HIRE_DATE),
       FLOOR(MONTHS_BETWEEN(SYSDATE, hire_date)) || '개월차' AS "근속개월"
FROM EMPLOYEE;

-- *ADD_MONTHS : 특정 날짜에 NUMBER개월수에 더해서 반환
...
-- *LAST_DAY(DATE) : 해당 월 의 마지막 날짜 구해서 반환
SELECT LAST_DAY(SYSDATE) FROM DUAL;


-- 사원테이블에서 사원명, 입사일, 입사달의 마지막날짜, 입사달의 근무일수 조회
SELECT EMP_NAME, HIRE_DATE, LAST_DAY(HIRE_DATE) AS "입사달의 마지막 날짜", LAST_DAY(HIRE_DATE)- HIRE_DATE AS "입사달 근무일수" 
FROM EMPLOYEE;
...
/*
    [형변환 함수]
    *TO_CHAR : 숫자 타입 또는 날짜 타입의 값을 문자타입으로 변환시켜수는 함수
...
SELECT TO_CHAR(1234, '99999') AS "NUMBER" FROM DUAL; -- 공간확보, 오른쪽 정렬, 빈칸공백  9 : 값이 없으면 표시안함
SELECT TO_CHAR(1234, '00000') AS "NUMBER" FROM DUAL; -- 0의 자리수만큼 공간확보, 오른쪽 정렬, 빈칸 0으로 채움  0: 값이 없으면 "0"으로 처리
...
SELECT TO_CHAR(SYSDATE, 'AM HH:MI:SS') FROM DUAL;--AM, PM 어떤것을 쓰던 형식에 맞춰나옴
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
        ...
        TO_CHAR(SYSDATE, 'RR'), --RR룰이 따로 존재한다 -> 50년이상값이 +100 -> EX) 1954
...
/*
    TO_DATE : 숫자타입 또는 문잩아입을 날짜타입으로 변경하는 함수
    
    TO_DATE(숫자 | 문자, [포멧]) -> DATE
*/

SELECT TO_DATE(20100101) FROM DUAL;
SELECT TO_DATE(20240219) FROM DUAL; -- 50년 미만은 자동으로  20XX으로 설정 50년 이상은 19XX로 설정된다.
...
--전사원의 이름, 보너스 포함 연봉 
SELECT EMP_NAME, SALARY, (SALARY + (SALARY * NVL(BONUS, 0))) * 12 AS "보너스 포함 연봉" 
FROM EMPLOYEE;
