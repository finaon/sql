/*
    ...
    (무작정 다 조회해 오는게 아니라 각 테이블간 연결고리(외래키)를 통해 매칭시켜 조회해야한다.)\
    
    JOIN은 크게 "오라클 전용구문"과 "ANSI 구문"(ANSI == 미국국립표준협회)
    ...
    ---------------------------------------------------------------------------         
            ...
            (LEFT OUTER)             |      외른쪽 외부 조인(RIGHT OUTER JOIN)
            (RIGHT OUTER)            |      전체 외부 조인 (FULL OUTER JOIN)
   -----------------------------------------------------------------------------
        자체조인(SELF JOIN)           |             JOIN IN
...
--1)연결할 두 컬럼명이 다른경우(EMP)(EMPLOYEE : DEPT_CODE / DEPARTMENT : DEPT_ID)
--전체 사원들의 사번, 사원명, 부서코드, 부서명을 조회
...
--2)연결할 두 컬럼명이 같은 경우(EMPLOYEE : JOB_CODE)
--전체사원들의 사번, 사원명, 직급코드, 직급명을 조회
...
--2.DEPARTMENT와 LOCATION테이블을 참고하여 전체 부서의 부서코드, 부서명, 지역코드, 지역명 조회
--> 오라클
...
--4. 부서가 총무부가 아닌 사원드리 사원명, 급여 조회
--> 오라크
SELECT EMP_NAME, SALARY
FROM EMPLOYEE, DEPARTMENT
WHERE EMP_CODE = DEPT_ID
AND DEPT_TITLE != '총무부';
--> ANSI
...
/*
    ...
    투 테이블간의 JOIN시 일치하지 않는 행도 포함시켜 조회 가능
...
--ANSI
SELECT EMP_NAME, DEPT_ID, DEPT_TITLE, SALARY, SALARY * 12
FROM EMPLOYEE
...
--3) FULL [OUTER] JOIN : 두 테이블이 가진 모든 행을 조회할 수 있다(오라클 X)
--ANSI 구문
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY * 12
FROM EMPLOYEE
...
SELECT E.EMP_ID AS "사원사번", E.EMP_NAME AS "사원명", E.DEPT_CODE,
        M.EMP_ID AS "사수사번", M.EMP_NAME AS "사수명", M.DEPT_CODE
FROM EMPLOYEE E
LEFT JOIN EMPLOYEE M ON (M.MANAGER_ID = E.EMP_ID);
