/*
    <트리거> 
    내가 지정한 테이블에 INSERT, UPDATE, DELETE 등 DML문에 의해 변경이 생길때
    (테이블에 이벤트가 발생했을 때)
    자동으로 매번 실행할 내용을 미리 정의해 둘 수 있다.
    
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE 후 곧바로 탈톼한 회원들만 따로 보관하는 테이블에
    자동으로 INSERT 시켜야한다.
    신고횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리되게끔 
    입출고에 대한 데이터가 기록(INSERT)될 때마다 해당 상품에대한 재고수량을 매번 수정(UPDATE)해야한다.
    
    
    *트리거 종류 
    -SQL문의 실행시기에 따른 분류
    >BEFORE TRIGGER : 내가 지정한 테이블에 이벤트가 발생되기 바로 전에 트리거 실행
    >AFTER TRIGGER : 내가 지정한 테이블에 이벤트가 발생된 후 트리거 실행하기
    
    -SQL문에 의해 영향을 받는 각 행에 따른 분류
    >문장트리가: 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행
...
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, DEPT_CODE, JOB_CODE, HIRE_DATE)
VALUES(501, '김유신', '111111-1111111', 'D7','J7', SYSDATE);
...
--1. 상품에 대한 데이터를 보고나할 테이블(TB_PRODUCT)
...
--샘플데이터 추가
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '갤럭시24', '샘송', 14000000, DEFAULT);
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '아이폰24', '사과', 13000000, 10);
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '대륙', '샤우미', 8000000, 20);

COMMIT;

--2. 상품 입출고 상세이력 테이블 생성(TB_PRODETAIL)
--어떤 상품이 어떤 날짜에 몇개가 입고 또는 출고가 되는지 데이터를 기록
CREATE TABLE TB_PRODETAIL(
    DECODE NUMBER PRIMARY KEY, --이력번호
    PCODE NUMBER REFERENCES TB_PRODUCT, --상품번호
    PDATE DATE NOT NULL, --출고일
...
--TB_PRODUCT테이블에 매번 자동으로 재고수량 UPDATE 되게끔 트리거 작성

/*
    -상품이 입고된 경우 -> 해당 상품을 찾아서 재고수량 증가 UPDATE
    UPDATE TB_PRODUCT
    SET STOCK = STOCK + 현재입고된 수량 (INSERT된 자료 AMOUNT)
    WHERE PCODE = 입고된 상품번호(INSERT된 자료 자료의 PCODE);
    -상품이 출고된 경우 -> 해당 상품을 찾아서 재고수량 증가 UPDATE
    UPDATE TB_PRODUCT
    SET STOCK = STOCK - 현재입고된 수량 (INSERT된 자료 AMOUNT)
    WHERE PCODE = 출고된 상품번호(INSERT된 자료 자료의 PCODE);
*/
