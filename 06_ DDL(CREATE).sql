...
/*
    1. 테이블 생성
    -테이블이란 : 행과 열로 구성되는 가장 기본적인 데이터베이스객체 
     ...
     * 자료형
     -문자(CHAR(바이트크기) | VARCHAR2(바이트크기))
         ...
         VARCHAR2 : 최대 4000바이트 까지 지정가능 / 가변길이(몇글자의 데이터가 담길지 모르는 경우)
         (담긴 값에 따라서 공간 크기가 맞춰침)
     -숫자(NUMBER)    
     -날짜(DATE)
...
/*
    <제약조건>
    -원하는 데이터값(유요한 형식의 값)만 유지하기 위해서 특정 컬럼에 설정하는 제약
    -데이터 무결성 보장을 목적으로 한다.
...
/*
    * NOT NULL 제약조건
    ...
    삽입/수정시 NULL값이 허용하지 않도록 제한
    
    제약조건을 부여하는 방식 크게 2가지가 있다.(컬럼레벨방식/ 테이블레벨방식)
...
INSERT INTO MEM_UNIQUE
VALUES(2, 'USER1', 'PASS1', '홍길동', '여', NULL, NULL);
...
INSERT INTO MEM_PRI
VALUES(1, 'USER2', 'PASS2', '홍길TNS', '여', NULL, NULL);
--> 기본키에 중복값을 담으려고 할 때(UNIQUE제약조건 위반)

INSERT INTO MEM_PRI
VALUES(NULL, 'USER2', 'PASS2', '홍길TNS', '여', NULL, NULL);
--> 기본키에 NULL을 담으려고 할때 (NOT NULL제약조건 위반)

INSERT INTO MEM_PRI
VALUES(2, 'USER2', 'PASS2', '홍길TNS', '여', NULL, NULL);
...
/*
    *FOREIGN KEY(외래키) 제약조건
    ...
    > 컬럼레벨방식
    컬럼명 자료현 REFERENCES 참조할 테이블명[(참조할 컬럼명)]

    >테이블레벨방식
    FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명[(참조할 컬럼명)]
    
    -> 참조할 컬럼명 생략시 참조할 테이블에 PRIMARY KEY로 지정된 컬럼이 매칭된다.
*/
...
--> 부모테이블로부터 무조건 삭제가 안되는 "삭제제한" 옵션이 걸려 있다
...
--INSERT INTO 테이블명 VALUES(컬럼값, 컬럼거ㅏㅄ..)
INSERT INTO MEMBER VALUES(1, '빰빵이', 20, '운동', '20/01/01');
INSERT INTO MEMBER VALUES(2, '옥지', 22, NULL, NULL);
INSERT INTO MEMBER VALUES(3, '최지원', 12, DEFAULT, DEFAULT);

SELECT * FROM MEMBER;

----INSERT INTO 테이블명 VALUES(컬럼값, 컬럼거ㅏㅄ..)
INSERT INTO MEMBER(MEM_NO, MEM_NAME) VALUES(4, '이광인');
--> 선택되지 않는 컬럼에는 기본적으로 NULL이 들어감
...
/*
    *테이블이 다 생성된 후에 뒤늦게 제약조건을 추가하는 방법
    ...
    -PRIMARY KEY : ALTER TABLE테이블명 ADD PRIMARY KEY(컬럼명);
    -FOREIGN KEY : ALTER TABLE테이블명 ADD FOREIGN KEY(컬럼명) REFERNCES 참조할 테이블명[(참조할컬럼명)];
    -UINQUE      : ALTER TABLE테이블명 ADD UNIQUE(컬럼명);
    -CHECK      : ALTER TABLE테이블명 ADD CHECK(컬럼에 대한 조건식);
    -NOT NULL    : ALTER TABLE테이블명 MODIFY 컬럼명 NOT NULL;
