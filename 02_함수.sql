...
/*
    ...
    - ASC : 오름차순(작은값으로 시작해서 값이 점점 커지는것)
    - DESC : 내림차순(큰값으로 시작해서 작은값으로 점점 내려가는것)
    
    
    -- NULL은 기본적으로 가장 큰값으로 분류해서 정렬한다.
    - NULLS FIRST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당데이터 맨 앞에 배치(DESC일때 기본값)
    - NULL LAST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당데이터 맨 마지막에 배치(ASC일때 기본값)
...
/* 
    ...
    -그룹함수 : N개ㅐ의 값을 읽어들여서 1개의 결과값을 리턴(그룹을 지어 그룹별로 함수 실행결과 반환)
...
/*
    ...
    *LENGTH(걸림 | '문자열') : 해당 문자열의 글자수를 반환
    *LENGTH(걸림 | '문자열') : 해당 문자열의 바이트수를 반환
...
/*
    *INSTR
    ...
    INSTR(걸림 : '문자열', '찾고자하는 문자', ['찾을 위치의 시작값', 순번]) => 결과는 NUMBER 나옴
*/

SELECT INSTR('AABAACAABBAA', 'B') 
FROM DUAL; -- 앞쪽에 있는 첫 B는 3번째 위치에 있다고 나옴)
...
SELECT INSTR('AABAACAABBAA', 'B', -1)  
FROM DUAL; -- 뒤에서부터 찾지만 읽을 때는 앞으로 읽어서 알려준다.

SELECT INSTR('AABAACAABBAA', 'B', 1, 2)  
FROM DUAL; -- 순번을 제시하려면 찾을 위치의 시작값을 표시해야함
...
SELECT EMP_NAME, SUBSTR(EMP_NO) AS "성별"
FROM EMPLOYEE;

-- 사원들중 여사원들만 조회
SELECT EMP_NAME, EMP_NO SUBSTR(EMP_NO) AS "성별"
FROM EMPLOYEE
...
SELECT LTRIM('ACABACCKH', 'ABC') FROM DUAL;--제거하고자하는 문자는 문자열이 아닌 문자들!
...
SELECT TRIM('Z' FROM 'ZZZZKHZZZZZZ') FROM DUAL; --양쪽에 있는 특정문자 제거

SELECT TRIM(LEADING 'Z' FROM 'ZZZZKHZZZZZZZ') FROM DUAL; -- LTRIM 유사한 기능
SELECT TRIM(TRAILING 'Z' FROM 'ZZZZKHZZZZZZZ') FROM DUAL; -- RTRIM유사한 기능
SELECT TRIM(BOTH 'Z' FROM 'ZZZZKHZZZZZZZ') FROM DUAL;--양쪽에 있는 특정문자 제거
...
SELECT ROUND(123.5056) FROM DUAL; --기본자리수는 소수점 첫번째 자리에서 반올림
...
--어떤 문제가 있는지 원인을 서술하고, 조사한 코드를 작성하세요.
